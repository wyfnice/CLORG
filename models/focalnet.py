...
class FocalNet(nn.Module):
    ...
    def forward_featuremaps(self, x):
        ...
        for layer in self.layers:
            x, H, W = layer(x, H, W)
            feat_list.append(x)  # 将当前层的特征添加到列表中

        x = self.norm(x)  # B L C
        return x, feat_list  # 返回特征和特征列表
    ...
    def forward(self, x):
        x, feat_list = self.forward_features(x)  # 获取特征和特征列表
        x = self.head(x)
        return x, feat_list  # 返回分类结果和特征列表
...
if __name__ == '__main__':
    ...
    if __name__ == "__main__":
        model = focalnet_tiny_srf()  # 我现在要改成64*64
