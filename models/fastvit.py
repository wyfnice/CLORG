...
from NewCL.CIFAR.mobileone import MobileOneBlock
from NewCL.CIFAR.replknet import ReparamLargeKernelConv
...
class FastViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize feat_list to store features
        self.feat_list = []
    ...
    def forward_tokens(self, x: torch.Tensor) -> torch.Tensor:
        outs = []
        feat_list = []  # 初始化用于存储特征的列表
        for idx, block in enumerate(self.network):
            x = block(x)
            feat_list.append(x)  # 存储每一层的特征
            if self.fork_feat and idx in self.out_indices:
                norm_layer = getattr(self, f"norm{idx}")
                x_out = norm_layer(x)
                outs.append(x_out)
        if self.fork_feat:
            return outs, feat_list  # 返回每一层的特征
        return x, feat_list
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # through backbone
        x, feat_list = self.forward_tokens(x)
        if self.fork_feat:
            return x, feat_list  # 输出特征和 feat_list
        # for image classification
        x = self.conv_exp(x)
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        cls_out = self.head(x)
        return cls_out, feat_list  # 返回分类结果和特征列表
