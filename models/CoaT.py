...
class CoaT(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=4, embed_dims=[0, 0, 0, 0],
    ...
    def forward_features(self, x0):
        ...
        feat_list.append(x1_nocls)  # 存储特征
        ...
        feat_list.append(x2_nocls)  # 存储特征
        ...
        feat_list.append(x3_nocls)  # 存储特征
        ...
        feat_list.append(x4_nocls)  # 存储特征
        ...
        if self.parallel_depth == 0:
            if self.return_interm_layers:
                ...
                return feat_out, feat_list  # 返回特征和feat_list
            else:
                x4 = self.norm4(x4)
                x4_cls = x4[:, 0]
                return x4_cls, feat_list  # 返回分类结果和feat_list
        ...
        if self.return_interm_layers:
            ...
            return merged_cls, feat_list  # 返回分类结果和feat_list

    def forward(self, x):
        if self.return_interm_layers:  # Return intermediate features (for down-stream tasks).
            feat_out, feat_list = self.forward_features(x)
            return feat_out, feat_list  # 返回特征和feat_list
        else:  # Return features for classification.
            x, feat_list = self.forward_features(x)  # 获取特征和feat_list
            x = self.head(x)  # 分类
            return x, feat_list  # 返回分类结果和feat_list
