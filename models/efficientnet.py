...
from NewCL.CIFAR._builder import build_model_with_cfg, pretrained_cfg_for_features
from NewCL.CIFAR._efficientnet_blocks import SqueezeExcite
from NewCL.CIFAR._efficientnet_builder import BlockArgs, EfficientNetBuilder, decode_arch_def, efficientnet_init_weights, \
    round_channels, resolve_bn_args, resolve_act_layer, BN_EPS_TF_DEFAULT
from NewCL.CIFAR._features import FeatureInfo, FeatureHooks, feature_take_indices
from NewCL.CIFAR._manipulate import checkpoint_seq
from NewCL.CIFAR._registry import generate_default_cfgs, register_model, register_model_deprecations
...
class EfficientNet(nn.Module):
    ...
    def forward_intermediates( # 前向传递并返回中间层的特征
            self,
            x: torch.Tensor,
            ...
            extra_blocks: bool = False, # 是否包括所有块和头部卷积的输出
        ...
        return x, intermediates # 如果intermediates_only 为true，返回中间特征列表，否则，返回最终输出和中间特征列表

    def prune_intermediate_layers( # 修剪不需要的中间层
            self,
        ...
        return take_indices # 返回需要保留的中间层索引

    def forward_features(self, x): # 前向传递并返回特征图
        x = self.conv_stem(x)
        x = self.bn1(x)
    ...
    def forward_head(self, x, pre_logits: bool = False): # 前向传递分类头部
        x = self.global_pool(x) # x：输入特征图 ，pre_logits：是否返回分类器前的特征图
        if self.drop_rate > 0.:
            x = F.dropout(x, p=self.drop_rate, training=self.training)
        return x if pre_logits else self.classifier(x)

    def forward(self, x): # 完整的前向传递 包括特征提取和分类
        feat_list = self.forward_intermediates(x)
        x = self.forward_features(x)
        x = self.forward_head(x)
        return x , feat_list #返回分类结果
