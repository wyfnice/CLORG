...
class MogaNet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            # 将每层的特征添加到 feat_list 中
            self.feat_list.append(x)

        if self.fork_feat:
            return self.feat_list  # 如果启用了 fork_feat，则返回每一层的特征
        else:
            return x
    ...
    def forward(self, x):
        ...
        if self.fork_feat:
            ...
            # 返回分类结果和 feat_list
            return self.forward_head(x), self.feat_list


def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 4, 'input_size': (3, 224, 224),
...
if __name__=="__main__":
    model=moganet_xtiny()#我现在要改成64*64
